<div class="p-8 max-w-[1400px] mx-auto">
    ...
    <div class="flex items-center justify-between mb-10">
        <h1 class="text-3xl font-bold text-[#14519A] flex items-center gap-3">
            <i class="fas fa-box"></i> Gestión de Productos
        </h1>
        <a href="/products/create"
            class="btn bg-[#14519A] hover:bg-[#0f3a70] text-white px-5 py-2 rounded-xl shadow-md flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php foreach ($productsPaginated['items'] as $p): ?>
            <div
                class="group card bg-white shadow-md hover:shadow-xl rounded-2xl border border-gray-100 hover:-translate-y-1 transition duration-300 overflow-hidden">

                <!-- Imagen -->
                <figure class="h-52 bg-gray-50 flex items-center justify-center">
                ...
                <div class="card-body px-5 py-6">
                    <!-- Nombre -->
                    <h2 class="text-lg text-[#14519A] font-semibold truncate">
                    ...
                    <div class="mt-4 flex justify-between items-center">
                        <span class="text-xl font-bold text-[#14519A]">
                    ...
                    <div class="flex gap-2 mt-5">
                        <a href="/products/edit/<?= $p->id_product ?>"
                            class="flex-1 btn btn-sm bg-[#14519A] hover:bg-[#0f3a70] text-white rounded-xl shadow transition">
                            <i class="fas fa-edit"></i> Editar
                        </a>
                        <a href="/products/delete/<?= $p->id_product ?>"
                            class="flex-1 btn btn-sm bg-red-600 hover:bg-red-700 text-white rounded-xl shadow transition">
...
<!-- Paginación -->
<div class="flex justify-center gap-2 my-12">
    <?php if ($productsPaginated['page'] > 1): ?>
        <a href="?page=<?= $productsPaginated['page'] - 1 ?>" class="px-4 py-2 rounded-full shadow-md transition text-sm font-semibold 
    ...
    <?php for ($i = 1; $i <= $productsPaginated['totalPages']; $i++): ?>
        <a href="?page=<?= $i ?>" class="px-4 py-2 rounded-full shadow-md transition text-sm font-semibold
